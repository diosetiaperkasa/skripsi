...
include("koneksi.php");
if(isset($_POST['cari'])){
$list_transaksi=mysql_query("SELECT `wilayah`.`Nama`, `transaksi_detail`.*, `barang`.`Nama_Barang` 
FROM `wilayah` INNER JOIN `transaksi_detail` ON `transaksi_detail`.`Wilayah` = `wilayah`.`Id_Wilayah`
...
<tr><td><select name="nama" class="form-control"/>
	...
	<?php  $data=mysql_query("select * from Wilayah");
...
<br>
<div style="text-align:right;"><a href="per_wilayah.php">Download Exel</a></div>
<h4>Laporan Wilayah PT. Bando Indonesia</h4>
