...
FROM
  ...
  INNER JOIN `barang` ON `transaksi_detail`.`Id_Barang` = `barang`.`Id_Barang` where salesman.id_Salesman ='".$_POST['nama']."'");
	//echo $list_transaksi;
}else{
	$list_transaksi=mysql_query("SELECT
  `salesman`.*,
...
<tr><td><select name="nama" class="form-control"/>
	...
	<?php  $data=mysql_query("select * from salesman");
...
<div style="text-align:right;"><a class="no-print" href="javascript:printDiv('print-area');">Print      </a><a class="no-print" href="excel_sales.php">  Download Excel</a> </div>
<h4>Laporan Komisi Sales PT. Bando Indonesia</h4>
