...
if(isset($_POST['save'])){
	$id=$_POST['status'];
	$bc="update transaksi SET counter='2' WHERE Id_Transaksi='$id'";
		$ggc=mysql_query($bc);
	if($ggc){
...
<td><?php echo $data_detail['komplain'];?></td>
<td>Rp.<?php echo number_format($data_detail['Harga'] * $data_detail['Qty']);?></td>
</tr>
<input type="hidden" name="status" value="<?php echo $data_detail['Id_Transaksi'];?>"/>
  <?php } ?>
  <tr>
...
<input type="submit" name="save" class="btn btn-danger" value="fix error">
</form>
</div>
